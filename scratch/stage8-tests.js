...
function parseInput(input) {
  ...
  if (!validPattern.test(normalized) && !validPattern2.test(normalized)) {
    throw new Error("D\xFCzg\xFCn olmayan format: Yaln\u0131z m\xFCsb\u0259t r\u0259q\u0259ml\u0259r q\u0259bul edilir");
  }
  let value;
  try {
    value = new decimal_default(normalized);
  } catch (error) {
    throw new Error("D\xFCzg\xFCn olmayan format: R\u0259q\u0259m emal edil\u0259 bilm\u0259di");
  }
  if (value.isNegative()) {
...
var HISTORY_KEY = "ismar_calc_history";
...
var CalculatorStore = class {
  listeners = /* @__PURE__ */ new Set();
  weightInput = storageService.get("ismar_weight") || "";
  priceInput = storageService.get("ismar_price") || "";
  result = null;
  totalOutput = "0.00";
  error = null;
  constructor() {
...
function expectThrow(fn, expectedMessage, testName) {
  try {
    fn();
    failed++;
...
assert(parseInput("0.0001").equals(1e-4), 'parse("0.0001") -> "0.0001"');
expectThrow(() => parseInput("-5"), "Yaln\u0131z m\xFCsb\u0259t r\u0259q\u0259ml\u0259r", 'parse("-5") throws Azerbaijani error');
expectThrow(() => parseInput(""), "Bo\u015F daxiletm\u0259", 'parse("") throws Azerbaijani error');
expectThrow(() => parseInput("."), "Yaln\u0131z m\xFCsb\u0259t r\u0259q\u0259ml\u0259r", 'parse(".") throws Azerbaijani error');
expectThrow(() => parseInput("1..13"), "Yaln\u0131z m\xFCsb\u0259t", 'parse("1..13") throws Azerbaijani error');
console.log("\n--- 2. Calculation Engine ---");
calculatorStore.setPrice("175");
calculatorStore.setWeight("1.13");
assert(calculatorStore.result.total?.equals(197.75) ?? false, "175 \xD7 1.13 = 197.75");
calculatorStore.setWeight("1,13");
assert(calculatorStore.result.total?.equals(197.75) ?? false, "175 \xD7 1,13 = 197.75");
calculatorStore.setWeight("0.001");
assert(calculatorStore.result.total?.equals(0.175) ?? false, "175 \xD7 0.001 = 0.175");
calculatorStore.setWeight("0.0001");
assert(calculatorStore.result.total?.equals(0.0175) ?? false, "175 \xD7 0.0001 = 0.0175");
calculatorStore.setPrice("173");
calculatorStore.setWeight("3.45");
assert(calculatorStore.result.total?.equals(596.85) ?? false, "173 \xD7 3.45 = 596.85");
calculatorStore.setPrice("175");
calculatorStore.setWeight("0");
assert(calculatorStore.result.total?.equals(0) ?? false, "0 \xD7 any valid weight = 0");
calculatorStore.setPrice("0");
calculatorStore.setWeight("2.5");
assert(calculatorStore.result.total?.equals(0) ?? false, "any valid price \xD7 0 = 0");
console.log("\n--- 3. State management ---");
...
calculatorStore.setWeight("-2");
assert(calculatorStore.error?.includes("m\xFCsb\u0259t") ?? false, "invalid values produce Azerbaijani errors");
console.log("\n--- 4. History engine ---");
